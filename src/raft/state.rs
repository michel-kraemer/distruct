...
use crate::raft::{ClientRequest, ClientResponse, TypeConfig};
...
pub struct StateMachine(Arc<StateMachineInner>);
...
impl RaftStateMachine<TypeConfig> for StateMachine {
        ...
        I: IntoIterator<Item = Entry>,
    {
        ...
        for entry in entries {
            ...
            match entry.payload {
                EntryPayload::Blank => res.push(ClientResponse { value: None }),
                EntryPayload::Normal(req) => match req {
                    ClientRequest::Set { key, value } => {
                        sm.data.insert(key, value.clone());
                        res.push(ClientResponse { value: Some(value) });
                    }
                },
                EntryPayload::Membership(mem) => {
                    sm.last_membership = StoredMembership::new(Some(entry.log_id), mem);
                    res.push(ClientResponse { value: None });
                }
            };
        }
...
impl RaftSnapshotBuilder<TypeConfig> for StateMachine {
    async fn build_snapshot(&mut self) -> Result<Snapshot<TypeConfig>, StorageError<NodeId>> {
        ...
        let snapshot_idx = self.0.snapshot_idx.fetch_add(1, Ordering::Relaxed) + 1;
